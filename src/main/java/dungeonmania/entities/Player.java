...
public class Player extends Entity implements Battleable, Overlap {
    ...
    public void onOverlap(GameMap map, Entity entity) {
        if (entity instanceof Enemy) {
            ...
            // if (isInvisible() || ((Assassin) entity).isAllied())
            // return;
            // }
            map.setBattle(this, (Enemy) entity);
        }
    }
    ...
    public List<BattleItem> getBattleItemInInventory(Class<BattleItem> clz) {
        return getInventory().getEntities(BattleItem.class);
    }

    public void setPlayerHealthToActualStatistics(double actualHealth) {
        getBattleStatistics().setHealth(actualHealth);
    }

    public void useWeapon(Game game) {
        getInventory().getWeapon().use(game);
    }
